class User < ApplicationRecord
  ...
  validates :last_name, presence: true, format: { with: /\A[ぁ-んァ-ン一-龯]+\z/ , message: "は全角文字（漢字・ひらがな・カタカナ）で入力してください" }
  validates :first_name, presence: true, format: { with: /\A[ぁ-んァ-ン一-龯]+\z/ , message: "は全角文字（漢字・ひらがな・カタカナ）で入力してください" }
